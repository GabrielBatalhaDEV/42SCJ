...
teclasOperadoras.forEach((tecla) =>
  tecla.addEventListener("click", selecionaOperador)
);
...
const calcular = () => {
  ...
  atualizarDisplay(String(resultado).replace(".", ","));
};
...
const decimal = () => {
  if (!display.textContent || numeroNovo) atualizarDisplay("0,");
